...
QMap<QString, QString> AppSettings::engineHeaders(QOnlineTranslator::Engine engine) const
{
    switch (engine) {
    case QOnlineTranslator::DeepLXFree: {
        const auto headerSettings(m_settings->value(QStringLiteral("Translation/DeepLXFreeHeaders")).value<QMap<QString, QVariant>>());
        QMap<QString, QString> headers;
        for (auto it = headerSettings.cbegin(); it != headerSettings.cend(); ++it)
...
void AppSettings::setEngineHeaders(QOnlineTranslator::Engine engine, const QMap<QString, QString> &headers)
{
    switch (engine) {
    case QOnlineTranslator::DeepLXFree: {
        QMap<QString, QVariant> headerSettings;
        for (auto it = headers.cbegin(); it != headers.cend(); ++it)
            headerSettings[it.key()] = it.value();

        m_settings->setValue(QStringLiteral("Translation/DeepLXFreeHeaders"), headerSettings);
        break;
    }
    default:
        Q_UNREACHABLE();
...
QMap<QString, QString> AppSettings::defaultEngineHeaders(QOnlineTranslator::Engine engine)
{
    switch (engine) {
    case QOnlineTranslator::DeepLXFree:
        return {};
...
QMap<QOnlineTranslator::Language, QLocale::Country> AppSettings::regions(QOnlineTranslator::Engine engine) const
{
    ...
    case QOnlineTranslator::DeepLX:
    case QOnlineTranslator::DeepLXFree: {
        // Shared by both engines: this is a translation-target preference (e.g. British vs.
        // American English), not a per-engine setting, so both read/write the same store.
