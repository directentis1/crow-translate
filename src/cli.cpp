...
void Cli::process(const QCoreApplication &app)
{
    ...
    const QCommandLineOption engine({"e", "engine"}, tr("Specify the translator engine ('google', 'yandex', 'bing', 'libretranslate', 'lingva', 'deeplx' or 'deeplxfree'), Google is used by default."), QStringLiteral("engine"), QStringLiteral("google"));
    ...
    parser.setApplicationDescription(tr("A simple and lightweight translator that allows to translate and speak text using Google, Yandex, Bing, LibreTranslate, Lingva, DeepLX and DeepLXFree"));
    ...
    } else if (parser.value(engine) == QLatin1String("deeplx")) {
